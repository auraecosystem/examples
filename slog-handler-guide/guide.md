...
Although this particular output is valid YAML, our implementation doesn't consider the subtleties of YAML
syntax, so it will sometimes produce invalid YAML.
...
It is unusual to use a `*sync.Mutex` instead of a `sync.Mutex`,
but there is a good reason which we'll explain later.
