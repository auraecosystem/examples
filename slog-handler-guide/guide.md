...
`Warn` and `Error`), and corresponding methods that take a context (`DebugCtx`,
`InfoCtx`, `WarnCtx` and `ErrorCtx`).
...
It will display a log output call like this:
...
return an error,
though you might want to report the problem through your production monitoring
...
a new `Kind`&mdash;a backwards-compatible change under the Go 1 Compatibility Promise&mdash;and
