...
1. Allocate a buffer, a `[]byte`, to hold the output.
It's best to construct the output in memory first,
then output it with a single call to `io.Writer.Write`,
...
happens when the initial slice is empty or small.
