...
`WithAttrs` calls, building up a slice of group names and attribute lists,
...
We add a slice of `groupOrAttrs` to our handler:
...
receiver. To that end, we define a method that will make copy our handler
struct and append one `groupOrAttrs` to the copy:
...
Using that function, the `With` methods are easy:
