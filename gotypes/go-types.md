...
introduced in Go 1.9, merely declares another name for an
...
This declaration does more than just give another name to a type.
It first defines a new `Named` type (also called `Celsius`)
...
types_ instead of named types, but the type checker API predates that
change.
