...
We've written this tutorial to help you find your bearings.
...
though that package will eventually cease to be maintained.
...
provides a robust recursive-descent parser.
...
run `go get github.com/golang/examples/gotypes/...`.
...
provides `FileSet`, a table that stores this information
...
The type checker is robust to ill-formed input, and reports accurate
...
Finally, the program prints the attributes of the package:
...
It is unique within a workspace, and for published packages it must
be globally unique.
...
The type checker reports an error if all the package declarations in
the package do not agree.
...
loaded from export data returns zero, also known as
`token.NoPos`.
...
`struct{T T}` from anonymous ones like `struct{T}`.
...
for that case.
The `default` case, the `nil` case, and cases with more than
...
The main job of the type checker is, of course, deducing the type
of each expression.
...
types, that is, the _nullary_ type constructors.
This is essentially the set of types that a constant expression is
...
All numeric `Value`s, whether integer or floating-point, signed
or unsigned, real or complex, are represented exactly using
infinite-precision integers or rational numbers.
(The Go spec requires only that the representation use at least 256 bits of precision.)
Arithmetic operations on `Value`s are exact too.
...
Second, it does not contain syntax trees nor semantic information
...
You must have enabled the `ParseComments` flag.
