...
func TestDotProduct(t *testing.T) {
	...
	dotGotUnrolled := dotProductGoUnrolled(va, vb)
	dotGotAVX2 := dotProductAVX2(va, vb)

	if !almostEqualF32(dotWant, dotGotScalar) {
		t.Errorf("scalar got %v, want %v", dotGotScalar, dotWant)
	}
	if !almostEqualF32(dotWant, dotGotUnrolled) {
		t.Errorf("unrolled got %v, want %v", dotGotUnrolled, dotWant)
	}
...
func BenchmarkDotProductUnrolled(b *testing.B) {
	aa := makeRandSlice(benchArrSize)
	bb := makeRandSlice(benchArrSize)

	b.SetBytes(benchArrSize * 4 * 2)
	for b.Loop() {
		dotProductGoUnrolled(aa, bb)
	}
}
