...
func main() {
	...
	classCfg := &weaviate.ClassConfig{
		Class:    "Document",
		Embedder: googleai.Embedder(embeddingModelName),
	}
	indexer, retriever, err := weaviate.DefineIndexerAndRetriever(ctx, *classCfg)
	if err != nil {
		log.Fatal(err)
	}
...
func (rs *ragServer) queryHandler(w http.ResponseWriter, req *http.Request) {
	...
	for _, part := range genResp.Candidates[0].Message.Content {
		respTexts = append(respTexts, part.Text)
	}
